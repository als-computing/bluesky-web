...
export default function QItemPopup( {popupItem={}, handleQItemPopupClose=()=>{}, isItemDeleteButtonVisible=true, handleCopyItemClick=()=>{} } ) {
    ...
    const Row = (name, icon, content) => {
        ...
        return (
            <div name="background" onClick={handleQItemPopupClose} className={`absolute top-0 left-0 w-full h-full z-20 rounded-md ${getPlanColorOpacity(popupItem.name)} flex justify-center items-center ${isDeleteModeVisible ? 'bg-red-600/40' : ''}`}>
                <div name="main popup" onClick={(e)=> e.stopPropagation()} className={`z-30 ${isHistory ? 'w-[90%] h-[70%] max-w-6xl max-h-[80rem]' : 'w-[75%]  h-[50%] min-h-[40rem] max-w-[40rem] max-h-[60rem]'} rounded-lg ${isDeleteModeVisible ? deleteBg : 'bg-slate-50'}`}>
