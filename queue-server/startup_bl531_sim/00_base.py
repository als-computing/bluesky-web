...
"""
SIMULATION VARIANT of startup_bl531, for docker-compose.sim.yml.

Differences from startup_bl531:
  - Tiled URI comes from $TILED_URI (default http://tiled:8000) instead of the
    hardcoded beamline address 192.168.10.155. Same change in
    16_plans_gisaxs.py, which opens a second client at module scope.
  - 02_area_detectors.py is omitted; caproto-server does not simulate the
    Basler or Pilatus area detectors.
Everything else is a verbatim copy. Keep it that way -- fix the real beamline
files first, then re-copy.
"""
...
tiled_client = from_uri(os.getenv("TILED_URI", "http://tiled:8000"), api_key=api_key)
