...
@app.get("/devices/{prefix}", status_code=200)
def describe_device(prefix, response: Response):
...
def initialize_device(prefix, response: Response):
    if prefix in device_dict:
        ...
        return {"201" : "Device " + prefix + " is connected"}
    
@app.put("/device/move", status_code=200)
async def move_device(instruction: DeviceInstruction, response: Response):
    if instruction.prefix in device_dict:
        device = device_dict.get(instruction.prefix)
        try:
            device.set(instruction.set_value).wait(timeout=1)
            return{"200" : "Instruction accepted, moved " + instruction.prefix + " to " + str(instruction.set_value) + "."}
        except Exception as error:
